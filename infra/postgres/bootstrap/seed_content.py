...
BLOG_POST_ROWS = [{'id': '155286fe-5710-53c7-be2d-6088b3bfa71c',
  ...
  'excerpt': 'A look at the homelab setup I use to learn more about storage, hosting, networking, and running my own '
             'tools.',
  'excerpt_nl': 'Een blik op de homelab-setup die ik gebruik om meer te leren over opslag, hosting, netwerken en '
                'het draaien van mijn eigen tools.',
  'content_markdown': '# My Homelab\n'
                      ...
                      '\n'
                      '![Diagram of the homelab setup](https://media.alex-vp.com/portfolio/blog/my-homelab/cover.png)\n'
                      '\n'
                      '\n'
  ...
  'content_markdown_nl': "# Mijn homelab\n\nIn het begin bestond mijn homelab alleen uit mijn TrueNAS-server. Sindsdien is het sterk gegroeid. Mijn huidige homelab is langzaam veranderd van een paar losse experimenten naar een persoonlijke, kleinschalige infrastructuuromgeving. Het is niet langer één server met willekeurige containers. De infrastructuur is duidelijk gescheiden per verantwoordelijkheid tussen containers en virtuele machines, publiek verkeer wordt gecontroleerd, en netwerkverkeer wordt consequent gemonitord. Daardoor kan ik projecten makkelijker deployen voor persoonlijk én publiek gebruik.\n\nHet idee achter deze setup is simpel: ik wilde een plek die aanvoelt als een kleine persoonlijke datacenteromgeving. Ze moet mijn eigen applicaties kunnen hosten, experimenten ondersteunen, gameservers draaien, opslag aanbieden, en mij een veilige omgeving geven om meer te leren over infrastructuur, DevOps, networking, monitoring en AI-deployment.\n\n![Diagram van de homelab-setup](https://media.alex-vp.com/portfolio/blog/my-homelab/cover.png)\n\n## De hoofdarchitectuur\n\nDe setup is opgebouwd rond een Proxmox-server waarop meerdere virtuele machines draaien. Elke VM heeft een specifieke rol, in plaats van alles in één groot systeem te stoppen.\n\nAan de rand van het netwerk stuurt mijn router alleen verkeer door dat echt het homelab binnen moet. Daarna wordt het verkeer opgesplitst op basis van het gebruik. Publiek webverkeer gaat via een edge-VM, gameserververkeer gaat rechtstreeks naar de gameserver-VM vanwege latency, en interne services communiceren via het lokale netwerk.\n\nEen aparte TrueNAS-server beheert de opslag. Zo blijft de opslaglaag gescheiden van de compute-laag, wat de setup overzichtelijker en makkelijker te begrijpen maakt.\n\n### VM1: Edge\n\nDe edge-VM is het belangrijkste toegangspunt voor de meeste webgebaseerde services.\n\nDeze VM draait:\n\n- Cloudflare Tunnel\n- Nginx Proxy Manager\n- WireGuard\n\nDeze VM is verantwoordelijk voor het doorsturen van verkeer van buitenaf naar de juiste interne service. In plaats van elke applicatie rechtstreeks bloot te stellen, kan ik verkeer via Cloudflare en Nginx Proxy Manager routeren.\n\nCloudflare Tunnel is handig omdat sommige services bereikbaar kunnen zijn zonder veel poorten op mijn router open te zetten. Nginx Proxy Manager handelt daarna de routing af op basis van domeinen en subdomeinen. Zo kan één subdomein naar een persoonlijke app wijzen, terwijl een ander subdomein naar een project op een andere VM gaat.\n\nWireGuard draait hier ook, zodat ik veilig terug kan verbinden met mijn thuisnetwerk wanneer ik niet thuis ben. Dat is nuttig voor interne dashboards, beheerpanelen en services die niet publiek toegankelijk moeten zijn.\n\nKort gezegd is deze VM de gecontroleerde toegangspoort tot mijn homelab.\n\n---\n\n### VM2: GitHub Runner\n\nDe tweede VM is volledig gericht op deployments.\n\nDeze VM draait een GitHub Actions runner. Het idee is dat mijn projecten automatisch vanuit GitHub gebouwd en gedeployed kunnen worden naar mijn interne app-VM.\n\nZo blijft het deploymentproces gescheiden van de applicaties zelf. In plaats van telkens handmatig via SSH in te loggen op de appserver wanneer ik een project wil bijwerken, kan ik wijzigingen naar GitHub pushen en de runner het deploymentwerk laten uitvoeren.\n\nDe runner communiceert met de app-VM en voert daar de deploymentstappen uit. Deze aanpak voelt dichter aan bij hoe echte CI/CD-pipelines in professionele omgevingen werken.\n\nEr is wel een kleine beperking: om veiligheidsredenen is de runner alleen beschikbaar binnen mijn eigen GitHub-organisatie. Daardoor werk ik voor deploybare projecten soms met twee repositories: één publieke repository voor ontwikkeling en één private repository die de deployment afhandelt.\n\n---\n\n### VM3: Apps\n\nDe apps-VM is de plek waar mijn eigen gehoste projecten draaien.\n\nDit omvat onder andere:\n\n- Het portfolioplatform\n- Webapplicaties\n- API's\n- Databases die bij die apps horen\n- Ondersteunende services, zoals koppelingen met mediaopslag\n\nDeze VM ontvangt verkeer van de edge-VM. Wanneer iemand bijvoorbeeld een van mijn publieke projectdomeinen bezoekt, sturen Cloudflare en Nginx Proxy Manager de aanvraag door naar de juiste applicatie op deze VM.\n\nSommige apps kunnen ook communiceren met de AI-VM wanneer ze LLM-functionaliteit nodig hebben. De apps-VM hoeft het AI-model dus niet zelf te draaien, maar kan een aanvraag naar de AI-service sturen en het antwoord gebruiken.\n\nDeze scheiding houdt de apps-VM gefocust op applicatiehosting, terwijl de AI-VM de zwaardere GPU-gerelateerde taken verwerkt.\n\n---\n\n### VM4: AI\n\nDe AI-VM wordt gebruikt voor lokale AI-workloads.\n\nZe draait momenteel vLLM en Ollama. Daardoor kunnen andere services in het homelab aanvragen doen naar een lokaal gehost taalmodel. Dat maakt het mogelijk om met AI-functionaliteit te experimenteren zonder volledig afhankelijk te zijn van externe API's.\n\nZowel de apps-VM als de persoonlijke-apps-VM kunnen de AI-VM aanspreken voor AI-gerelateerde use cases. Een webapplicatie kan bijvoorbeeld een prompt naar de AI-service sturen en het antwoord in de app gebruiken.\n\nAI-workloads op een aparte VM houden is belangrijk omdat ze veel resources kunnen vragen. Het maakt GPU-passthrough, geheugengebruik en model serving ook makkelijker apart te beheren van de rest van de infrastructuur.\n\n---\n\n### VM5: Gameservers\n\nDe gameserver-VM staat los van de webapplicatiekant van het homelab.\n\nDeze VM host gameservers, zoals Terraria, Minecraft of andere games.\n\nIn tegenstelling tot webapps hebben deze services vaak directe port forwarding vanaf de router nodig. Gameservers gebruiken meestal specifieke poorten en protocollen, waardoor hun verkeerspad verschilt van de normale Cloudflare/Nginx-webrouting. Bovendien kan extra latency door omleidingen via andere services nadelig zijn bij games waar reactietijd belangrijk is.\n\nDoor gameservers op hun eigen VM te houden, blijven ze geïsoleerd van de rest van de infrastructuur. Als een gameserver veel resources gebruikt of opnieuw moet worden opgestart, heeft dat geen invloed op mijn gehoste apps, monitoringstack of AI-services.\n\n---\n\n### VM6: Ops en monitoring\n\nDe ops-VM is waar ik de observability-kant van het homelab beheer.\n\nDeze VM draait:\n\n- Prometheus\n- Grafana\n- Loki\n- Alertmanager\n- Uptime Kuma\n\nDeze VM helpt me begrijpen wat er in de hele omgeving gebeurt.\n\nPrometheus verzamelt metrics. Grafana visualiseert die in dashboards. Loki wordt gebruikt voor logs. Alertmanager kan meldingen versturen wanneer er iets fout gaat. Uptime Kuma geeft een eenvoudig overzicht van welke services online of offline zijn.\n\nDit is een van de nuttigste onderdelen van de setup. Zodra er meerdere services draaien, wordt het moeilijk om alles handmatig te controleren. Monitoring geeft mij één centrale plek om de gezondheid van de infrastructuur te bekijken.\n\nIk kan bijvoorbeeld opvolgen of containers draaien, of een VM bereikbaar is, hoeveel RAM of CPU wordt gebruikt, en of publieke services correct reageren.\n\n---\n\n### VM7: Persoonlijke apps\n\nDe persoonlijke-apps-VM is bedoeld voor services die meer privé of persoonlijk zijn.\n\nDit kan bijvoorbeeld gaan om:\n\n- Homepage-dashboards\n- Nextcloud\n- Persoonlijke tools\n- Applicaties die alleen intern bereikbaar zijn\n\nDeze VM ontvangt ook verkeer van de edge-VM wanneer dat nodig is. Sommige services mogen publiek zijn, maar andere zijn alleen bedoeld om via WireGuard bereikbaar te zijn.\n\nNet zoals de apps-VM kunnen persoonlijke apps de AI-VM aanspreken wanneer ik AI-functionaliteit aan private tools wil toevoegen.\n\nDeze VM geeft mij een plek om software te draaien die nuttig is in het dagelijks leven, zonder die te mengen met publieke projectdeployments.\n\n---\n\n### TrueNAS-server\n\nOpslag wordt afgehandeld door een aparte TrueNAS-server.\n\nDe TrueNAS-server biedt opslag via SMB voor mijn Windows-machines en via NFS voor mijn Linux-laptop en andere machines.\n\nHier kan grotere persistente opslag leven. In plaats van alles rechtstreeks in elke VM op te slaan, kunnen VMs netwerkopslag gebruiken wanneer dat zinvol is.\n\nHet grootste voordeel van deze scheiding is dat Proxmox de compute-laag beheert, terwijl TrueNAS de opslaglaag beheert. Dat maakt de infrastructuur modulairder. Als ik een VM moet aanpassen of opnieuw opbouwen, hoeft de data niet noodzakelijk binnen die VM te staan.\n\nHet maakt backups, shares en opslagbeheer ook makkelijker te centraliseren.",
  ...
  'seo_description': 'A casual walkthrough of the homelab setup I use to learn self-hosting, storage, networking, and '
                     'deployment.',
  'seo_description_nl': 'Een toegankelijke rondleiding door mijn homelab-setup, waarmee ik leer over self-hosting, '
                        'opslag, netwerken en deployment.',
