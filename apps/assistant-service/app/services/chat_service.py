...
class ChatService:
    ...
    def respond(self, *, message: str, conversation_id: str | None = None, session_id: str | None = None) -> dict:
        ...
        retrieved = self.retrieval.search(message)
        citations = [
            CitationOut(
                title=item.title,
        ...
        context_blocks = [f'[{index + 1}] {item.title} ({item.source_type})\n{item.excerpt}' for index, item in enumerate(retrieved)]

        generated = None
        try:
            generated = self.provider.generate_answer(question=message, context_blocks=context_blocks, history=history)
        except Exception:
            generated = None

        answer = generated or self._build_fallback_answer(question=message, citations=citations)
    ...
    def _build_fallback_answer(self, *, question: str, citations: list[CitationOut]) -> str:
        if not citations:
            return (
                "I couldn't find enough indexed portfolio content to answer that yet. "
