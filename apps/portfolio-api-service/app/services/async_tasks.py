...
ADMIN_TASK_STATUS_VALUES = ('queued', 'running', 'succeeded', 'failed')
GITHUB_REFRESH_TASK = 'github-refresh'
KNOWLEDGE_REBUILD_TASK = 'assistant-knowledge-rebuild'
...
class AdminTaskQueue:
    def __init__(self, redis_url: str, *, ttl_seconds: int = 3600 * 24) -> None:
        ...
        self.queue_key = 'portfolio:admin:tasks:queue'
        self.processing_key = 'portfolio:admin:tasks:processing'
        self.task_key_prefix = 'portfolio:admin:tasks:'
    ...
    def enqueue(self, task_type: str, payload: dict[str, Any]) -> AdminTaskRecord:
        client = self._client()
        task_id = str(uuid4())
        submitted_at = _now_iso()
        envelope = AdminTaskEnvelope(task_id=task_id, task_type=task_type, payload=payload, submitted_at=submitted_at)
        ...
        try:
            ...
            raise TaskQueueUnavailable('Redis task queue is unavailable.') from exc
        return AdminTaskRecord(task_id=task_id, task_type=task_type, status='queued', submitted_at=submitted_at)
    ...
    def reserve(self, *, timeout_seconds: int = 5) -> tuple[AdminTaskEnvelope, str] | None:
        client = self._client()
        try:
    ...
    def mark_succeeded(self, task_id: str, *, result: dict[str, Any] | None, raw_item: str) -> None:
    ...
    def _client(self) -> Redis:
        if not self.enabled:
            raise TaskQueueUnavailable('Redis task queue is disabled.')
...
@lru_cache
def get_admin_task_queue() -> AdminTaskQueue:
    settings = get_settings()
    return AdminTaskQueue(settings.redis_url)


def reset_admin_task_queue_cache() -> None:
    get_admin_task_queue.cache_clear()
